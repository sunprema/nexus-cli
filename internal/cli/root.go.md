...
summary: "Builds the flattened `nexus` root command, wires in every subcommand, and sets the version Cobra reports for --version."
...
`map`, `tour`, the hidden `__post-commit-hook`, and the hidden `mcp` (MCP
stdio server). This is the single place the CLI's shape is assembled. It
also sets Cobra's `Version` field from `versioninfo.Version`, which is what
makes `nexus --version` work.
